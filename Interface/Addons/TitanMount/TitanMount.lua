...
function TitanPanelMountFavPanel_OnLoad(self)
		...
		if mountNamePassed == mountName then
			...
			for j = 1, table.getn(alphabet) do
				if alphabet[j].letter == letter then
					...
					DetectUsableMounts()
					print(MountC4..mountNamePassed.." was added to your Titan Panel [Mount] collection")
...
do
	...
	--Initializes display of favorite panel when it is shown
	function ShowFavoriteAddMenu()
		local function initialize(self, level)
			if level == 1 then
				for i = 1, table.getn(alphabet) do
					if alphabet[i].found then
					UIDropDownMenu_AddButton(ShowMenuButton(alphabet[i].letter), 1)
					end
				end
			elseif level == 2 then
				for i = 1, table.getn(alphabet) do
					--Match the MENU_VALUE to the letter in the alphabet
					if _G["UIDROPDOWNMENU_MENU_VALUE"] == alphabet[i].letter then
						--Prepare the mounts!
...
function TitanPanelRightClickMenu_PrepareMountMenu()
        if _G["L_UIDROPDOWNMENU_MENU_LEVEL"] == 2 then
			if _G["L_UIDROPDOWNMENU_MENU_VALUE"] == "Display Options" then
				local info = {}       
				...
				for i = 1, table.getn(alphabet) do
					--Match the MENU_VALUE to the letter in the alphabet
					if _G["L_UIDROPDOWNMENU_MENU_VALUE"] == alphabet[i].letter then
						--Prepare the mounts!
		...
		elseif _G["L_UIDROPDOWNMENU_MENU_LEVEL"] == 3 then
			...
			for i = 1, table.getn(alphabet) do
				--Match the MENU_VALUE to the letter in the alphabet
				if strupper(strsub(_G["L_UIDROPDOWNMENU_MENU_VALUE"], 1, 1)) == alphabet[i].letter then
					--Get the number from the MENU_VALUE
					local subNumber = strsub(_G["L_UIDROPDOWNMENU_MENU_VALUE"], -1, string.len(_G["L_UIDROPDOWNMENU_MENU_VALUE"]))
			...
			for i = 1, table.getn(alphabet) do
				if alphabet[i].found then
				DDM:UIDropDownMenu_AddButton(ShowMenuButton(alphabet[i].letter), 1)
				end
			end
